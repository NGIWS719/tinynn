...
class Model:
    ...
    # 预测概率值所对应的标签索引
    def predicted_label(self, x, train_flg=False):
        y = self.predict(x, train_flg)
        predicted_labels = np.argmax(y, axis=1)
        return predicted_labels

    # 计算准确度，x是输入数据，y是正确解标签
    def accuracy(self, x, y):
        predicted_labels = self.predicted_label(x, train_flg=False)
        if y.ndim != 1:
            y = np.argmax(y, axis=1)

        acc = np.sum(predicted_labels == y) / predicted_labels.size
        return acc

    # 测试集准确率
    def test(self, x_test, y_test):
        x_test = imgReshape(x_test, size=(32, 32))
    ...
    def train_step(self):
        ...
        if self.sample_batches:
            ...
            y_batch = self.y_batch[self.current_iter % self.iter_per_epochs - 1]
        else:
    ...
    def split_dataset(self):
        ...
        y_random_train = self.y_train[batch_mask]
        # 拆分数据
        self.x_batch = np.array(np.array_split(
            x_random_train, self.iter_per_epochs), dtype=object)
        self.y_batch = np.array(np.array_split(
            y_random_train, self.iter_per_epochs), dtype=object)
